...
# Fetch GitHub repository details (used for prefixing duplicate secrets)
...
locals {
  ...
  # Process all variables, ensuring unique names and correct Key Vault assignment
  all_variables = {
    for key, value in data.gitlab_project_variable.variables :
    (
      contains(local.existing_secrets, lower(replace(key, "[^a-zA-Z0-9-]", "-")))
      ? lower(replace("${data.github_repository.repo.name}-${key}", "[^a-zA-Z0-9-]", "-"))  # Use GitHub repo name as prefix for duplicates
      : lower(replace(key, "[^a-zA-Z0-9-]", "-"))
    ) => {
      value       = value.value
      environment = lookup(value, "environment_scope", "unknown")
      repo_name   = lower(replace(data.github_repository.repo.name, "[^a-zA-Z0-9-]", "-"))
    }
  }
}
...
resource "azurerm_key_vault_secret" "all_secrets" {
  ...
  content_type = each.value.repo_name  # Store the GitHub repo name in content_type for identification
  ...
  tags = {
    ...
    repository  = each.value.repo_name    # Store GitHub repo name as a tag
  }
